...
import SunPopupTokenIcon from "./SunPopupTokenIcon";
...
function SunPopup({ isWin }: SunPopupProps) {
  const { lastWinAmount, betCount } = useGameStore(
    useShallow((state) => ({
  ...
  const balanceFromStore: string | number = formatNumber(amountToShow);
